...
const server = http.createServer((request, response) => {
  ...
  const shouldRedirectHost = canonicalHost && hostName && hostName !== canonicalHost;
  const shouldRedirectProto = forceHttps && forwardedProto === "http";

  if (shouldRedirectHost || shouldRedirectProto) {
    const targetProtocol = forceHttps ? "https" : url.protocol.replace(":", "");
    const targetHost = canonicalHost || requestHost;
    send(response, 301, { Location: `${targetProtocol}://${targetHost}${url.pathname}${url.search}` }, "");
    return;
  }
